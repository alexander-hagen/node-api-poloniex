  ...
  async request(key, options) {
    ...
    if (this._ws.readyState === WebSocket.OPEN) {
      ...
      return new Promise((resolve, reject) => {
        ...
        this._promises.set(key, { resolve, reject, timeout });
...
//  terminate() {
//    console.log("Terminate socket "+this.name);
//    if(this._ws!==null) {
//      this._ws.emit('closed');
//      this._ws.terminate();
//    };
...
PoloniexSocket.prototype.clearHandlers = async function() {
  await this.socket.clearHandlers();
};

PoloniexSocket.prototype.query = async function(options) {
...
PoloniexSocket.prototype.getQuery = async function(path,query) {
  var options = {
    method: "GET",
...
PoloniexSocket.prototype.otherQuery = async function(method,path,query) {
  var options = {
    method: method,
    url: this.endPoint + path,
    params: {},
...
PoloniexSocket.prototype.createOrder = async function(opts) {
  const key="ReqID"+(++this.socket._id);
  const options={
    "id": key,
    "event":"createOrder",
    "params": opts
